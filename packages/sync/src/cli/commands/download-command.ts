...
import { buildObjectKey, parseObjectKey, parseSyncManifest } from '@lucasschirm/sal-sync-core';

import { buildStorageAdapter } from '../common.js';
 ...
 *
 * SECURITY: this is the path-traversal guard restored by #354 — the
 * pre-hoist `devin-session-sync` copy of this function had silently dropped
 * this comment (though not the guard logic itself) during the initial
 * copy-paste, and devin's test suite had zero regression coverage for it
 * (see `packages/sync/tests/unit/cli/commands/download-command.test.ts`'s
 * `refuses to write outside the output directory` test, which now exercises
 * this exact guard for every harness built on this shared implementation).
 */
...
function printUsage(adapter: CliHarnessAdapter, stderr: NodeJS.WritableStream): void {
  stderr.write(
    `Usage: ${adapter.binName} download --session-id=<id> --output=<dir>\n` +
      `       ${adapter.binName} download all --output=<dir>\n`,
  );
}
...
async function resolveDownloadConfig(
  ...
  const merged = env ?? (await resolveCliEnv(adapter, cwd));
  const validation = validateCliConfig(adapter, merged, cwd);
 ...
 *
 * Hoisted (#354) from `claude-session-sync`/`devin-session-sync`.
 */
export async function runDownloadCommand(
  ...
  if (!args) {
    printUsage(adapter, stderr);
    return 1;
  }

  const config = await resolveDownloadConfig(adapter, cwd, options.env, stderr);
  if (!config) return 1;
