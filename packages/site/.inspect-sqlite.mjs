...
for (const path of ['/tmp/opfs-after-create.sqlite', '/tmp/opfs-after-reload.sqlite']) {
  ...
  capi.sqlite3_deserialize(db.pointer, 'main', pMem, n, n, capi.SQLITE_DESERIALIZE_FREEONCLOSE | capi.SQLITE_DESERIALIZE_RESIZEABLE);
  ...
  try {
    console.log(path, '-> projects rows:', JSON.stringify(db.selectObjects('SELECT * FROM projects')));
  } catch (e) { console.log(path, '-> query error:', e.message); }
  db.close();
}
