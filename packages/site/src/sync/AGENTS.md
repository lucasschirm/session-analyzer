...
- **sync-manager.ts** — Main-thread sync state machine: project/session discovery, worker queue, manifest diffing, and lifecycle hooks (including the default `onFileDownloaded` wired to `file-processing-bridge.ts`).
