...
- **storage-optimize.spec.ts** — UX-025: Storage settings page per-row Optimize/Download overlay terminal success/failure states, the Size column's loading/ok/error distinction, and the ~30s stall-safety-net transition (via `page.clock`).
