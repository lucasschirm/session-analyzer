...
- **storage-optimize.spec.ts** — UX-032: Storage settings page per-row Optimize/Download overlay terminal success/failure states, the Size column's loading/ok/error distinction, and the ~30s stall-safety-net transition (via `page.clock`).
- **sync.spec.ts** — CAS sync pipeline, offline handling, retry, passkey, and large transcript flows.
- **sync-fixtures.ts** — Mock S3 CAS payload and `FixtureBucket` builders for sync tests. The listing response emits `<LastModified>` alongside a content-derived (sha256-of-body, not key-derived) `<ETag>` per entry, matching the manifest-fingerprint discovery mechanism. `addSession({ gzip: true })` stores objects gzip-compressed with `Content-Encoding: gzip` (manifest hashes still cover the uncompressed content), mirroring sync-plugin uploads. `reuploadSessionManifest` (manifest-only bump, no file-content change), `updateSessionFile` (re-uploads a file's content and rebuilds the session manifest so both the body and the manifest's listing fingerprint change), and listing-level `setHttpError('list:<prefix>', …)` injection let tests simulate a manifest-only re-upload, a manifest+content re-upload, and a project listing failure respectively.
