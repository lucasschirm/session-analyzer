...
- **Canonical Subsession Ingestion**: Ingestion persists multiple `SessionSummary` records per session bundle (root and child subsessions) into `sessions` and `session_summaries`, with parent-child relationships preserved in `session_relations`. Main session `root_only` metrics measure strictly root-agent activity; child subsessions measure their own activity.
