...
function printSummary(
  ...
  stdout.write('\n');
  stdout.write(
    `Downloaded ${metrics.downloaded} file(s), ${formatBytes(metrics.totalBytes)} total, ${metrics.failed} failed to ${output}\n`,
  );
}
...
function printUsage(stderr: NodeJS.WritableStream): void {
  stderr.write(
    'Usage: devin-sync download --session-id=<id> --output=<dir>\n' +
