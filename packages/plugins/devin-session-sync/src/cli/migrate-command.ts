 ...
 * `DevinCliAdapter`, whose `migrateManifestHarness` correctly IS
 * `DevinHarnessProfile.harness` (`'devin'`) — see `devin-cli-adapter.ts`'s
 * doc comment. Preserves the exact `runMigrateCommand(argv, options?)`
...
export async function runMigrateCommand(
  ...
  return sharedRunMigrateCommand(DevinCliAdapter, argv, options);
}
