...
`FAILING-PRODUCT-BUG`. Every entry below was `GREEN` as of the original
backfill (issue #160); an entry with a later `Status` other than `GREEN`
is a new surface registered after the backfill, tracked in §9 until the
PR that implements it flips it to `GREEN`.
...
| UX-034 | Session Evidence context-growth chart renders real per-message token buckets matching the session's model requests | `session-analytics.spec.ts` | `expectRenderedGeometry` + accessible-table row assertions on exact context/generation token labels | 4 | 5 | 4 | 80 | P0 | IMPLEMENTING |
| UX-035 | Session Evidence utilization panel splits the model-sent tool set into available/used/unused | `session-analytics.spec.ts` | table cell + used/unused pill assertions against prompt_snapshot/tool_use fixture | 4 | 5 | 4 | 80 | P0 | IMPLEMENTING |
| UX-036 | Cherry-picked sessions render resolved titles (parsed ai-title or first-prompt-derived fallback, never the raw remote session id) once the targeted sync completes | `sync.spec.ts` | resolved-label assertion on `.session-title` pre-sync (date fallback, never raw id) and post-sync (`ai_title` resolved via `getSessionTitle` for both a real ai-title and a `fallbackTitle`-derived title) + `Not synced`→`Synced` badge transition | 4 | 4 | 4 | 64 | P0 | GREEN |
| UX-037 | Devin session context-growth chart is non-flat when the ATIF transcript carries per-step `model_usage` metrics (the flat-chart regression from per-step usage records lacking `parentId` turn linkage) | `devin-journey.spec.ts` | `expectRenderedGeometry` + accessible-table row assertions on distinct context-token levels across two model-switch steps (glm-5-2 -> swe-1-7) | 4 | 5 | 4 | 80 | P0 | IMPLEMENTING |
| UX-038 | Devin session context drawer shows the selected assistant message body (not "No content recorded") after the transformer carries `payload.content` on message evidence and the context-series encoder stores `cnt` per point | `devin-journey.spec.ts` | drawer open on accessible-table row click + `Message Content` section renders the fixture assistant text (markdown-rendered) | 4 | 5 | 4 | 80 | P0 | IMPLEMENTING |
| UX-039 | Devin session utilization panel reports an invoked tool, skill, and agent as **Used**: the transformer emits `component_evidence_link` records for root-session invocations (the only "used" signal `session_component_stats` reads) and gives every invoked component an identity, so a builtin like `exec` that no promoted availability list offers is no longer invisible. Closes the earlier DEFERRED follow-up: no `invocations` table write was needed — attribution runs through the link records. | `devin-journey.spec.ts` | Available/Used/Unused table cells per domain + `.pill.used` / `.pill.unused` membership + `#component-activity` row per used component with its real invocation count, labeled `kind/nativeId` and never the raw component id | 4 | 5 | 4 | 80 | P0 | GREEN |
| UX-040 | Cherry-pick page surfaces failed sessions: failed rows render the light-red `row-failed` background and red `Failed` badge with a `View error` action whose `session-error-modal` shows the stored `INGEST_FAILED`/`DOWNLOAD_FAILED` `sync_details` log (Escape/Close dismiss, focus moves into the dialog via `ModalBase`); transcript-less sessions (`transcriptsCaptured` with no main transcript artifact) surface an amber `No transcript` badge with the same View error dialog ("Main transcript not uploaded") plus a sync warning toast, never a silent "Not synced"; a subsequent bulk sync without "Include sessions failed importing" skips failed sessions (no manifest/transcript GET for them), while the cherry-pick retry path still syncs them | `sync.spec.ts` | `.row-failed`/`.badge-failed` assertion on the failed row + `View error` → `session-error-modal` dialog text assertion (error details, never only the raw id) + Escape dismiss + request-log assertion that the bulk re-run issues zero session-manifest GETs for the failed session + cherry-pick retry re-fetches the failed session's manifest (fixture uses unique transcript content so CAS dedup can't mask the failure) + `transcriptsCaptured: false` session renders `.badge-transcript-unavailable` and the "Main transcript not uploaded" dialog | 4 | 4 | 3 | 48 | P1 | GREEN |
| UX-041 | Devin context-growth bars distinguish Tool / Skill / Agent messages from plain ones: the transformer classifies each node from its own `chat_message.tool_calls` (and a tool-result node from the `tool_call_id` it answers), the series stores it per point, and the chart tints the `Context` segment per domain | `devin-journey.spec.ts` | legend swatches (`Message` / `Tool call` / `Skill call` / `Agent call`) + accessible-table row labels repeating the domain in text, so color is never the only carrier (`components/charts/AGENTS.md`); a plain message keeps its pre-existing label verbatim | 4 | 4 | 4 | 64 | P1 | GREEN |
| UX-042 | The message drawer toggles between the parsed (markdown) body and the raw message record as formatted JSON, defaulting to parsed and resetting per selection | `devin-journey.spec.ts` | `Parsed`/`Raw` `aria-pressed` state + `pre.raw-json` content parses back to the message object and carries the canonical fields (`messageId`, `contextTokens`, `invocationKind`) + switching back re-renders the markdown | 4 | 4 | 4 | 64 | P1 | GREEN |
| UX-043 | The `Session duration (min)` card is presented in whole minutes (`1.667 minutes` -> `2 minutes`), display-side only | `devin-journey.spec.ts` | `.value` matches `/\d+ minutes/` and contains no decimal separator | 3 | 4 | 4 | 48 | P1 | GREEN |
...
| PIPE-021 | Ingest-time context-growth series (#430): `session_context_series` row materialized inside the atomic commit, `getContextTimingSeries` served without touching `normalized_events`, pointer-only `raw_details` skeletons, signed compaction encoding, superseded-generation replacement, and per-session write-failure isolation | `pipe-021-context-series.test.ts` | `session_context_series` row-shape + `getContextTimingSeries` post-purge equality + skeleton-shape + negative-encoding + generation-replacement assertions | 4 | 5 | 4 | 80 | P0 | GREEN |
| PIPE-022 | Devin transcript-only context growth (real session `lucky-squid`, anonymized): no ATIF / null `response_dimensions` → `numTokensPreceding` checkpoint feeds `session_context_series`; compaction boundaries whose `summarized_from` outputs live in detached continuation trees still emit `normalized_event` compaction records | `pipe-022-devin-context-growth.test.ts` | `getContextTimingSeries` non-null/non-flat context assertions matching the fixture's real checkpoints, exact 5-record compaction count, `devin:tokens:*` stays null (missing-is-never-zero) | 4 | 5 | 4 | 80 | P0 | GREEN |
| PIPE-023 | Devin transcript-only context growth from per-request `chat_message.metadata.metrics` (no ATIF, real `brassy-humor` defect): per-message `model_request` records feed the chart, the cumulative `response_dimensions` aggregate is never attributed to a single message (directly or via `requestOrder`), and `devin:tokens:total` stays on the harness aggregate | `pipe-023-devin-per-message-context.test.ts` | `getContextTimingSeries` per-request context (`input + cacheRead`), no session-scale context point or compaction, `devin:tokens:total` == `(input + cached) + output` | 4 | 5 | 4 | 80 | P0 | GREEN |
| PIPE-024 | Devin component usage attribution reaches the read contract: `component_evidence_link` records aggregate into `session_component_stats`, so `getUtilizationReport` reports invoked components as Used, `getComponentFacts` returns per-component invocation counts with `kind/nativeId` labels, and the context series carries the per-message Tool/Skill/Agent domain | `pipe-024-devin-component-usage.test.ts` | session `sessionDomains` available/used/unused counts and component labels, `getComponentFacts` rows, `getContextTimingSeries` `invocationKind` per point, project report `usedSessions` vs declared-but-unused | 4 | 5 | 4 | 80 | P0 | GREEN |
