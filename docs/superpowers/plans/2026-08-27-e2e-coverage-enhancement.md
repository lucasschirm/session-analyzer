...
| UX-040 | Cherry-pick page surfaces failed sessions: failed rows render the light-red `row-failed` background and red `Failed` badge with a `View error` action whose `session-error-modal` shows the stored `INGEST_FAILED`/`DOWNLOAD_FAILED` `sync_details` log (Escape/Close dismiss, focus moves into the dialog via `ModalBase`); a subsequent bulk sync without "Include sessions failed importing" skips the failed session (no manifest/transcript GET for it), while the cherry-pick retry path still syncs it | `sync.spec.ts` | `.row-failed`/`.badge-failed` assertion on the failed row + `View error` → `session-error-modal` dialog text assertion (error details, never only the raw id) + Escape dismiss + request-log assertion that the bulk re-run issues zero session-manifest GETs for the failed session + cherry-pick retry re-fetches the failed session's manifest (fixture uses unique transcript content so CAS dedup can't mask the failure) | 4 | 4 | 3 | 48 | P1 | GREEN |
