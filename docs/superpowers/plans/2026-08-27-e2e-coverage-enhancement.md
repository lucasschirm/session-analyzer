...
| UX-039 | Devin session utilization panel reports tool invocations (EditFile) as available and used — FOLLOW-UP: the ingestion pipeline does not currently populate the `invocations` table from `invocation` records, so the component-utilization panel and component-facts section show empty states for Devin sessions. Surfacing Devin tool calls requires ingestion pipeline changes to write `invocation` records to the `invocations` table and/or produce `configuration_snapshot` records for tool availability. | `devin-journey.spec.ts` | (deferred — requires ingestion pipeline changes to populate `invocations` table from `invocation` records) | 4 | 4 | 4 | 64 | P1 | DEFERRED |
