...
| UX-036 | Cherry-picked session renders its parsed ai-title (never the raw remote session id) once the targeted sync completes | `sync.spec.ts` | resolved-label assertion on `.session-title` pre-sync (date fallback, never raw id) and post-sync (`ai_title` resolved via `getSessionTitle`) + `Not synced`→`Synced` badge transition | 4 | 4 | 4 | 64 | P0 | GREEN |
