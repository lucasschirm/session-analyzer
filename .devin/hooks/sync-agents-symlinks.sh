#!/bin/bash
# SessionStart hook: ensures .devin/<name> is a symlink to .agents/<name> for
...
# elsewhere in this repo); .devin/<name> is a compatibility symlink so
# Devin CLI, which reads under .devin/, transparently sees the same
# content.
#
# Runs on every session start, so it must be fast, non-interactive, and safe
# to re-run. For each pair:
#   - .devin/<name> is already a symlink resolving to .agents/<name>: no-op.
#   - .devin/<name> is missing entirely: .agents/<name> is created as an
#     empty directory if it doesn't already exist, then .devin/<name> is
...
#   - .devin/<name> is a symlink pointing somewhere else (stale/wrong): the
...
#   - .devin/<name> exists as a real file or directory with actual content
...
# not Devin CLI pipes one in.
cat >/dev/null

# Resolve the project root reliably regardless of the directory the hook is
# invoked from. Devin CLI exports DEVIN_PROJECT_DIR to hook commands; fall
# back to the script's own on-disk location (.devin/hooks/<this>.sh is
# always two directories below the project root) so the script also works
# when run/tested directly outside of a Devin CLI session.
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" >/dev/null 2>&1 && pwd)"
PROJECT_ROOT="${DEVIN_PROJECT_DIR:-$(cd "$SCRIPT_DIR/../.." >/dev/null 2>&1 && pwd)}"
...
sync_one() {
  ...
  if [ -L "$devin_link" ]; then
    ...
    rm -f "$devin_link"
    mkdir -p "$agents_dir"
    ln -s "$rel_target" "$devin_link"
    echo "sync-agents-symlinks: fixed stale symlink .devin/$name -> $rel_target" >&2
    return 0
  fi

  if [ -e "$devin_link" ]; then
    # Real file or directory with content - never delete or overwrite.
    echo "sync-agents-symlinks: WARNING: .devin/$name exists and is real content (not a symlink)." >&2
    echo "sync-agents-symlinks:   Leaving it untouched. To resolve: move/merge its contents into" >&2
    echo "sync-agents-symlinks:   .agents/$name, delete .devin/$name, then start a new session." >&2
    return 0
  fi
